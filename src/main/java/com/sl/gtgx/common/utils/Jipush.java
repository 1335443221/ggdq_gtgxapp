package com.sl.gtgx.common.utils;
...
public class Jipush {
	...
	public static final String APPKEY = "6aed3734b048d1bc007bc2a8";
	public static final String MASTERSECRET = "f45f57c86d15a5bbc0429f00";
	...
	/**
	 * TODO modify hlc on 2019-8-9 12:06:35 修改目标为 tag = "env=dev", alias = 用户集合 修复bugID = 407
	 */
	private static PushPayload buildPushObjectByAlias(String content, Map<String, String> params, List<String> userIds) {
		...
		//必须配置env tag才行, 否则会推送给其他环境 id 相同的用户
		builder.setAudience(Audience.newBuilder()
				.addAudienceTarget(AudienceTarget.tag_and(UrlConfig.ENV_TEST))
	     ...
	     * 根据别名集合发送
	     */
	    public static int sendByAliasList(String content, Map<String, String> params,List<String> users) {
	        ClientConfig clientConfig = ClientConfig.getInstance();
	        JPushClient jpushClient = new JPushClient(MASTERSECRET, APPKEY, null, clientConfig);
	        PushPayload payload = buildPushObjectByAlias(content, params, users);
	        int r=2;
	...
	//极光推送>>Android
	public static int jpushAndroidToAll(String alert) {
		//创建JPushClient(极光推送的实例)
		JPushClient jpushClient = new JPushClient(MASTERSECRET, APPKEY);
		//推送的关键,构造一个payload
		PushPayload payload = PushPayload.newBuilder()
	   ...
	   /**
	     ...
	     * @return 0推送失败，1推送成功
	     */
	    public static int sendToTagList(List<String> tagsList, String msg_content,String extra, Map<String,String> extrasparam,String notification_title) {
	    	ClientConfig clientConfig = ClientConfig.getInstance();
	    	JPushClient jpushClient = new JPushClient(MASTERSECRET, APPKEY, null, clientConfig);
	    	int result = 0;
	        try {
	    ...
	    /**
	     *向所有平台单个或多个指定Tag用户推送消息---通知
	     * @param tagsList
	     * @param msg_content
	     * @param extrasparam
	     * @return
	     */
	    private static PushPayload buildPushObject_all_tagList_alertWithTitle(List<String> tagsList,  String msg_content,String extraKey, Map<String,String> extrasparam,String notification_title) {
...
//						.build())
    ...
    public static void main(String[] args) {
		...
		extras.put("activeId",String.valueOf("1"));
		tagList.add("env=dev");
