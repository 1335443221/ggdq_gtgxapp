...
import com.sl.gtgx.app_1.entity.Malfunction;
import com.sl.gtgx.app_1.entity.MalfunctionWithBLOBs;
import com.sl.gtgx.app_1.entity.MalfunctionLog;
import com.sl.gtgx.app_1.entity.MalfunctionLogWithBLOBs;
import com.sl.gtgx.app_1.entity.Material;
import com.sl.gtgx.app_1.service.MalfunctionService_app1;
...
import com.sl.gtgx.manager.service.impl.CommonDataServiceImpl;
...
import static com.sl.gtgx.common.config.UrlConfig.*;
import static com.sl.gtgx.common.utils.TimePeriodUtil.getTimePeriod;
...
public class MalfunctionService_app1Impl implements MalfunctionService_app1 {
    ...
    public Result1 getMalfunctionList(Map<String, Object> params) {
        ...
        int total = 0;
        /*//department_id=2时（管理部），故障只有上报人和7位管理部主管可见
        if((departmentId==2&&(rid==3||rid==4))||(departmentId!=2)){
            //isCharge=1 表示可见全部门
            total = malfunctionMapper.selectMalfunctionListCount(statusList,fixBys,departmentId,1);
        }else{
            //isCharge=0表示仅可见自己上报的故障
            total = malfunctionMapper.selectMalfunctionListCount(statusList,fixBys,departmentId,0);
        }*/
        if(rid==1||rid==2||(departmentId==2&&rid==3)){
            //isCharge=2 表示公司领导可见is_company_manager=1的未处理故障
            total = malfunctionMapper.selectMalfunctionListCount(statusList,uid,fixBys,departmentId,2,companyId);
        }else{
            //isCharge=3 表示其他情况
            total = malfunctionMapper.selectMalfunctionListCount(statusList,uid,fixBys,departmentId,3,companyId);
        }
        ...
        if(total>0){
            if(rid==1||rid==2||(departmentId==2&&rid==3)){
                malfunctions = malfunctionMapper.selectMalfunctionList(statusList,uid,limitNum,create_at,fixBys,departmentId,2,companyId);
            }else{
                malfunctions = malfunctionMapper.selectMalfunctionList(statusList,uid,limitNum,create_at,fixBys,departmentId,3,companyId);
            }
