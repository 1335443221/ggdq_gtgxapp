package com.sl.gtgx.app_1.service.impl;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.sl.gtgx.app_1.dao.*;
import com.sl.gtgx.app_1.entity.Patrol;
import com.sl.gtgx.app_1.entity.PatrolLog;
import com.sl.gtgx.app_1.entity.User;
import com.sl.gtgx.app_1.service.PatrolService_app1;
import com.sl.gtgx.common.Interceptor.GlobalExceptionHandler;
...
import static com.sl.gtgx.common.utils.TimePeriodUtil.*;
import static com.sl.gtgx.common.utils.JwtToken.*;
...
public class PatrolService_app1Impl implements PatrolService_app1 {
     ...
     * @return
     */
    public Integer getTimeTypeNow(Long time,int departmentId) {
        Date date = new Date();
        if(time!=0L){
        ...
        //计算出早晚班后，还要根据部门id到事件类型表中查找对应的time_type_id
        if(timeType==1){
            if(departmentId==1){
                timeType = 1;
       ...
       // List<Integer> uIds = new ArrayList<>();
        if(rid==1||rid==2){
          ...
          //  uIds = userMapper.selectUidsByCompanyId((Integer)user.get("company_id"));
            ...
            timeType = getTimeTypeNow(0L,departmentId);
            companyId=null;
            //巡检计划可以在部门范围内可见
        ...
        int total = patrolMapper.selectPatrolCount(companyId,start,end,timeType,departmentId,
                ...
                monthPeriod.get("startTime"),monthPeriod.get("endTime")).size();
        ...
        List<Map<String, Object>> patrolList = patrolMapper.selectPatrols(
                ...
                weekPeriod.get("startTime"),weekPeriod.get("endTime"),
                monthPeriod.get("startTime"),monthPeriod.get("endTime"));
        //遍历每个巡检
        for (Map<String, Object> patrol : patrolList) {
            //若没有第一次打卡记录，则已打卡次数clock_count=0
            if (patrol.get("first_clock_at") == null && patrol.get("second_clock_at") == null) {
                patrol.put("clock_count", 0);
            }
            //若存在第一次打卡记录没有第二次打卡记录，则已打卡次数clock_count=1
            if (patrol.get("first_clock_at") != null && patrol.get("second_clock_at") == null) {
                patrol.put("clock_count", 1);
            }
            //若存在第二次打卡记录没有第三次打卡记录，则已打卡次数clock_count=2
            if ( patrol.get("second_clock_at") != null && patrol.get("third_clock_at") == null)  {
                patrol.put("clock_count", 2);
            }
            //若存在第三次打卡记录没有第四次打卡记录，则已打卡次数clock_count=3
            if ( patrol.get("third_clock_at") != null && patrol.get("fourth_clock_at") == null)  {
                patrol.put("clock_count", 3);
            }
            //若存在第一次打卡记录，则已打卡次数clock_count=4
            if ( patrol.get("third_clock_at") != null && patrol.get("fourth_clock_at") != null)  {
                patrol.put("clock_count", 4);
            }
        }
    ...
    public Result1 addPatrol(Map<String, Object> params) {
        ...
        if(alaisList.contains(alais)){
            return new Result1("1009","本班组该时段和巡检点已存在巡检任务",rid);
        }
        ...
        patrol.setDepartmentId((Integer)user.get("department_id"));
    ...
    public Result1 getPatrolLog(Map<String, Object> params) {
        ...
        for (Map<String, Object> result : patrolLogs) {
            //time是最后一次打卡时间
            Integer time = ((BigInteger) result.get("time")).intValue();
    ...
    public Result1 setClockOnce(Map<String, Object> params) {
        ...
        int timeType = getTimeTypeNow(0L,(Integer)user.get("department_id"));
    ...
    public Result1 setLotClock_1(Map<String, Object> params) {
        ...
        if(clocks!=null&&clocks.size()>0){
            ...
            for(Map clock:clocks){
                ...
                int timeType = getTimeTypeNow(time,departmentId);
    ...
    public Result1 getAllPatrolTimes(String token) {
        ...
        List<Map<String,Object>> patrolTimes = patrolTypeMapper.selectPatrolTimes
                (Integer.parseInt(user.get("department_id").toString()));
    ...
    public Result1 setLotClock(Map<String, Object> params) {
        ...
        if(clocks!=null&&clocks.size()>0){
            for (Map<String, Object> clock : clocks) {
                ...
                //先检测该巡检的巡检状态
                Map<String,Object> patrolLogMap = patrolLogMapper.selectByPatrolId(patrol_id, startTime, endTime,0L,0L
                ,0L,0L);
                //若没有该巡检记录，则新增一条巡检记录
    ...
    Result1 isLegalByPatrolId(Integer patrolBy,String token, int patrolId, String alais, int count,Long time) {
        ...
        Map<String,Object> patrolLogMap = patrolLogMapper.selectByPatrolId
                (patrolId, startTime, endTime,weekStart,weekEnd,monthStart,monthEnd);
        //若该巡检还未完成，生成新的巡检记录
        if (patrolLogMap == null) {
            ...
            if(patrolBy!=null){
                ...
                patrolLog_new.setPatrolBy(userId);
            }
            //根据总次数设置是否已完成巡检
            if (count == 1) {
            ...
            //存入 first
            patrolLog_new.setFirstClockAt(unixTimestamp.intValue());
            patrolLogMapper.insertSelective(patrolLog_new);
            logger.info(JSONObject.toJSONString(patrolLog_new));

            //若已有该巡检的巡检记录，判断此次打卡的有效性
        } else {
            //判断打卡次数
            //if ((patrolLog.get("fourth_clock_at")!=null)||(patrolLog.get("second_clock_at") != null && count==2)||count==1) {
            ...
            Long diff = 0L;
            if(patrolLogMap.get("second_clock_at")==null){
                diff = (unixTimestamp - (Long) patrolLogMap.get("first_clock_at"));
            }else if(patrolLogMap.get("second_clock_at")!=null&&patrolLogMap.get("third_clock_at")==null){
                diff = (unixTimestamp - (Long) patrolLogMap.get("second_clock_at"));
            }else if(patrolLogMap.get("third_clock_at")!=null&&patrolLogMap.get("fourthlock_at")==null){
                diff = (unixTimestamp - (Long) patrolLogMap.get("third_clock_at"));
            }
            ...
            //条件通过, 存入指定字段
            if(patrolLogMap.get("second_clock_at")==null){
                patrolLog.setSecondClockAt(unixTimestamp.intValue());
                if(count==2){
                    patrolLog.setStatus(2);
                }
            }else if(patrolLogMap.get("second_clock_at")!=null&&patrolLogMap.get("third_clock_at")==null){
                patrolLog.setThirdClockAt(unixTimestamp.intValue());
                if(count==3){
                    patrolLog.setStatus(2);
                }
            }else if(patrolLogMap.get("third_clock_at")!=null&&patrolLogMap.get("fourthlock_at")==null){
                patrolLog.setFourthClockAt(unixTimestamp.intValue());
                patrolLog.setStatus(2);
            }
